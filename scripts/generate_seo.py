...
SERVICES = [
    ...
    ('stroitelstvo-iz-keramzitobloka-v',
     'Дома из керамзитоблока в {prep}',
     'Строительство домов из керамзитоблока в {prep}. Тёплый, дышащий и долговечный материал — оптимальный по цене для постоянного проживания.',
     'keramzit'),
    ('karkasnye-doma-v',
     'Каркасные дома в {prep}',
     'Каркасное домостроение в {prep} — быстро, тепло и доступно. Срок строительства от 2 месяцев, технология подходит для постоянного проживания на юге.',
     'karkas'),
    ('doma-iz-penobloka-v',
     'Дома из пеноблока в {prep}',
     'Дома из пеноблока в {prep}: лёгкие тёплые стены, простая обработка, доступная цена. Подходит для одно- и двухэтажных построек.',
     'penoblok'),
    ('doma-iz-arbolita-v',
     'Дома из арболита в {prep}',
     'Арболитовые дома в {prep} — экологичный композит из щепы и цемента. Тёплый, не горит, держит гвозди, не требует доп. утепления.',
     'arbolit'),
    # Типы / планировки
    ('doma-s-mansardoy-v',
     'Дома с мансардой под ключ в {prep}',
     'Строительство домов с мансардой в {prep}. Полноценный второй уровень при цене одноэтажного, экономия земли и удобные спальни наверху.',
     'mansard'),
    ('doma-s-garazhom-v',
     'Дома с гаражом под ключ в {prep}',
     'Дома с встроенным или пристроенным гаражом в {prep}. Экономия на отдельной постройке, удобный выход из дома в любую погоду.',
     'garage'),
    ('doma-s-terrasoy-v',
     'Дома с террасой под ключ в {prep}',
     'Дома с террасой в {prep} — комфортное место для отдыха на юге. Открытая или крытая терраса под общей кровлей, проект на ваш выбор.',
     'terrace'),
    # По площади (популярные запросы)
    ('doma-100-m2-v',
     'Дома 100 м² под ключ в {prep}',
     'Готовые проекты домов 100 м² для строительства в {prep}. Оптимальная площадь для семьи из 3–4 человек — спальни, гостиная-кухня, санузел.',
     'size_100'),
    ('doma-120-m2-v',
     'Дома 120 м² под ключ в {prep}',
     'Проекты домов 120 м² в {prep}. Просторная планировка с 3 спальнями, гостиной-кухней и террасой. Срок строительства от 4 месяцев.',
     'size_120'),
    ('doma-150-m2-v',
     'Дома 150 м² под ключ в {prep}',
     'Дома 150 м² в {prep} — комфорт для большой семьи. 3–4 спальни, два санузла, гостиная-столовая и кабинет. Газобетон, кирпич, монолит.',
     'size_150'),
    ('doma-200-m2-v',
     'Дома 200 м² под ключ в {prep}',
     'Большие дома 200 м² под ключ в {prep}. Двухэтажный или одноэтажный дом для просторной жизни — продуманные планировки и большие террасы.',
     'size_200'),
    # Бюджет / класс
    ('nedorogie-doma-v',
     'Недорогие дома под ключ в {prep}',
     'Доступные дома под ключ в {prep} — газобетон или каркас, базовая комплектация и фиксированная цена. Эконом-формат от 2 400 000 ₽.',
     'cheap'),
    ('doma-ekonom-klassa-v',
     'Дома эконом-класса в {prep}',
     'Эконом-дома под ключ в {prep}: оптимизированный проект, рациональные материалы, без переплат за лишние метры. Качество подтверждено договором.',
     'econom'),
    ('doma-komfort-klassa-v',
     'Дома комфорт-класса в {prep}',
     'Дома комфорт-класса в {prep}: продуманные планировки, качественные материалы, грамотная инженерия. Для тех, кто хочет жить, а не выживать.',
     'comfort_class'),
    ('kottedzhi-pod-klyuch-v',
     'Коттеджи под ключ в {prep}',
     'Строительство коттеджей в {prep}: индивидуальный проект под ваш стиль жизни, премиальная отделка и инженерия, гарантия качества.',
     'kottedzh'),
    # Отдельные работы
    ('stroitelstvo-fundamenta-v',
     'Фундамент для дома в {prep}',
     'Устройство фундаментов в {prep}: ленточный, плитный, свайно-винтовой. Геология участка, расчёт нагрузок, армирование по проекту.',
     'foundation'),
    ('stroitelstvo-krovli-v',
     'Кровля для дома в {prep}',
     'Монтаж кровельных систем в {prep}: металлочерепица, фальцевая кровля, мягкая черепица. Стропильная система, утепление, водосточка.',
     'roof'),
]
...
THEME = {
    ...
    'keramzit':    ['Керамзитоблок 400×200×200 мм', 'Теплопроводность 0,16–0,2 Вт/м·К', 'Морозостойкость F50–F75', 'Армопояс по периметру и под перекрытием', 'Облицовка штукатуркой, кирпичом или вентфасад'],
    'karkas':      ['Каркас из сухого бруса 150×50 мм', 'Утепление минватой 200 мм по контуру', 'OSB-3 + ветрозащита + контробрешётка', 'Срок строительства от 2 месяцев', 'Свайно-винтовой или ленточный фундамент'],
    'penoblok':    ['Пеноблок D500/D600 толщиной 300 мм', 'Лёгкий — нагрузка на фундамент минимальна', 'Простая обработка инструментом', 'Обязательное утепление и облицовка', 'Армирование каждого 3-4 ряда сеткой'],
    'arbolit':     ['Композит из щепы и цемента', 'Не горит, не гниёт, экологичен', 'Держит крепёж без дюбелей', 'Утепление не требуется при стене 300 мм', 'Можно без облицовки — покраска по штукатурке'],
    'mansard':     ['Полноценный второй уровень под кровлей', 'Утепление кровли минватой 200 мм', 'Мансардные окна или фронтонные', 'Лестница на второй этаж в проекте', 'Цена ниже двухэтажного на 15–25%'],
    'garage':      ['Встроенный или пристроенный гараж', 'Размер на 1 или 2 машиноместа', 'Общая стена с домом — экономия на отоплении', 'Автоматические ворота на выбор', 'Подъездная площадка из брусчатки или бетона'],
    'terrace':     ['Открытая или крытая терраса', 'Под общей кровлей с домом', 'Деревянный или композитный настил', 'Перила, ограждения, ступени', 'Электричество и розетки на улицу'],
    'size_100':    ['2–3 спальни, гостиная-кухня, санузел', 'Газобетон, кирпич или каркас', 'Срок строительства от 3,5 месяцев', 'Площадь застройки 110–120 м²', 'Под ключ — от 4 200 000 ₽'],
    'size_120':    ['3 спальни, гостиная-столовая, 2 санузла', 'Газобетон 375 мм или кирпич 510 мм', 'Терраса 14–18 м² под общей кровлей', 'Площадь застройки 135–145 м²', 'Под ключ — от 5 100 000 ₽'],
    'size_150':    ['3–4 спальни, гардеробная, 2 санузла', 'Газобетон или кирпич, монолитный армопояс', 'Терраса, котельная, прихожая', 'Площадь застройки 170 м²', 'Под ключ — от 6 400 000 ₽'],
    'size_200':    ['4–5 спален, два уровня или просторный один', 'Гараж, кабинет, столовая отдельно', 'Качественные окна и инженерия', 'Площадь застройки 220+ м²', 'Под ключ — от 9 500 000 ₽'],
    'cheap':       ['Эконом-комплектация без переплат', 'Газобетон 300 мм или каркас', 'Базовая отделка под чистовую', 'Простая, но качественная инженерия', 'Фиксированная цена в договоре'],
    'econom':      ['Оптимизированный типовой проект', 'Газобетон D500 толщиной 300 мм', 'Металлочерепица, пластиковые окна', 'Базовая электрика и сантехника', 'Срок строительства 3–4 месяца'],
    'comfort_class':['Индивидуальные планировки', 'Многослойные стены с утеплителем', 'Премиальные окна, инженерия', 'Тёплый пол, рекуперация воздуха', 'Премиальная фасадная отделка'],
    'kottedzh':    ['Индивидуальный проект под ваш стиль', 'Премиальные материалы и инженерия', 'Качественная отделка под ключ', 'Ландшафтная подготовка участка', 'Гарантия 10 лет на конструктив'],
    'foundation':  ['Геология участка перед расчётом', 'Ленточный, плитный, свайно-винтовой', 'Бетон B25 с пластификаторами', 'Армирование арматурой А500С', 'Гидроизоляция и утепление цоколя'],
    'roof':        ['Стропильная система из бруса', 'Утепление минватой 150–200 мм', 'Гидро- и пароизоляция', 'Металлочерепица, фальц или мягкая кровля', 'Водосточная система и снегозадержатели'],
}
...
PRICE = {
    ...
    'keramzit':    ['Коробка керамзитоблок 100 м² — от 3 600 000 ₽', 'Дом керамзитоблок 145 м² — от 5 800 000 ₽', 'Дом керамзитоблок 180 м² — от 7 200 000 ₽'],
    'karkas':      ['Каркас 60 м² — от 2 100 000 ₽', 'Каркас 100 м² — от 3 400 000 ₽', 'Каркас 145 м² — от 4 800 000 ₽'],
    'penoblok':    ['Коробка пеноблок 100 м² — от 3 400 000 ₽', 'Дом пеноблок 145 м² — от 5 600 000 ₽', 'Дом пеноблок 180 м² — от 7 000 000 ₽'],
    'arbolit':     ['Коробка арболит 100 м² — от 4 100 000 ₽', 'Дом арболит 145 м² — от 6 500 000 ₽', 'Дом арболит 180 м² — от 8 200 000 ₽'],
    'mansard':     ['Газобетон с мансардой 130 м² — от 5 800 000 ₽', 'Кирпич с мансардой 160 м² — от 8 200 000 ₽', 'Каркас с мансардой 110 м² — от 4 400 000 ₽'],
    'garage':      ['Дом с гаражом 130 м² — от 6 100 000 ₽', 'Дом с гаражом 160 м² — от 7 800 000 ₽', 'Дом с гаражом на 2 авто 180 м² — от 9 200 000 ₽'],
    'terrace':     ['Дом с террасой 110 м² — от 5 200 000 ₽', 'Дом с террасой 145 м² — от 6 800 000 ₽', 'Дом с террасой 180 м² — от 8 400 000 ₽'],
    'size_100':    ['Газобетон 100 м² — от 4 200 000 ₽', 'Кирпич 100 м² — от 4 800 000 ₽', 'Каркас 100 м² — от 3 400 000 ₽'],
    'size_120':    ['Газобетон 120 м² — от 5 100 000 ₽', 'Кирпич 120 м² — от 5 900 000 ₽', 'Монолит 120 м² — от 6 400 000 ₽'],
    'size_150':    ['Газобетон 150 м² — от 6 400 000 ₽', 'Кирпич 150 м² — от 7 600 000 ₽', 'Монолит 150 м² — от 8 200 000 ₽'],
    'size_200':    ['Газобетон 200 м² — от 9 500 000 ₽', 'Кирпич 200 м² — от 10 800 000 ₽', 'Монолит 200 м² — от 11 500 000 ₽'],
    'cheap':       ['Газобетон 80 м² — от 3 200 000 ₽', 'Каркас 100 м² — от 3 400 000 ₽', 'Пеноблок 100 м² — от 3 400 000 ₽'],
    'econom':      ['Эконом газобетон 100 м² — от 4 200 000 ₽', 'Эконом каркас 80 м² — от 2 800 000 ₽', 'Эконом керамзитоблок 110 м² — от 4 400 000 ₽'],
    'comfort_class':['Комфорт газобетон 145 м² — от 6 100 000 ₽', 'Комфорт кирпич 110 м² — от 7 200 000 ₽', 'Комфорт монолит 150 м² — от 8 200 000 ₽'],
    'kottedzh':    ['Коттедж 150 м² — от 7 900 000 ₽', 'Коттедж 200 м² — от 11 500 000 ₽', 'Коттедж премиум 250 м² — от 15 000 000 ₽'],
    'foundation':  ['Ленточный фундамент — от 8 000 ₽/п.м', 'Плита 100 м² — от 480 000 ₽', 'Свайно-винтовой 80 м² — от 220 000 ₽'],
    'roof':        ['Металлочерепица 100 м² — от 380 000 ₽', 'Фальц 100 м² — от 520 000 ₽', 'Мягкая кровля 100 м² — от 340 000 ₽'],
}
...
HERO_PHOTO = {
    ...
    'keramzit':    'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%A2%D0%B1%D0%B8%D0%BB%D0%B8%D1%81%D0%BA%D0%B0%D1%8F%201.JPG',
    'karkas':      'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%A1%D0%9D%D0%A2%20%D0%97%D0%B0%D1%80%D1%8F%201.JPG',
    'penoblok':    'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%A1%D0%9D%D0%A2%20%D0%9D%D0%B5%D0%BF%D1%82%D1%83%D0%BD%201.JPG',
    'arbolit':     'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%A2%D0%B1%D0%B8%D0%BB%D0%B8%D1%81%D0%BA%D0%B0%D1%8F%202.JPG',
    'mansard':     'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%93%D0%B5%D0%BB%D0%B5%D0%BD%D0%B4%D0%B6%D0%B8%D0%BA%204.jpg',
    'garage':      'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%9A%D1%80%D0%B0%D1%81%D0%BD%D0%BE%D0%B4%D0%B0%D1%80%202.JPG',
    'terrace':     'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%93%D0%B5%D0%BB%D0%B5%D0%BD%D0%B4%D0%B6%D0%B8%D0%BA%203.jpg',
    'size_100':    'https://raw.githubusercontent.com/alekyanrazmik-glitch/Projekt/main/%D0%A1%D0%B5%D0%BC%D0%B5%D0%B8%CC%86%D0%BD%D1%8B%D0%B8%CC%86%202.JPG',
    'size_120':    'https://raw.githubusercontent.com/alekyanrazmik-glitch/Projekt/main/%D0%9F%D1%80%D0%BE%D0%B5%D0%BA%D1%82%20%D1%82%D0%BE%D0%BB%D1%8C%D1%8F%D1%82%D0%B8%201.JPG',
    'size_150':    'https://raw.githubusercontent.com/alekyanrazmik-glitch/Projekt/main/%D0%9F%D1%80%D0%BE%D0%B5%D0%BA%D1%82%20%D1%81%D0%B8%D0%BC%D1%84%D0%BE%D0%BD%D0%B8%D1%8F%201.JPG',
    'size_200':    'https://raw.githubusercontent.com/alekyanrazmik-glitch/Projekt/main/%D0%98%D0%BB%D1%8C%D1%81%D0%BA%D0%B8%D0%B8%CC%86%202.JPG',
    'cheap':       'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%9F%D0%B0%D0%B2%D0%BB%D0%BE%D0%B2%D1%81%D0%BA%D0%B0%D1%8F%201.JPG',
    'econom':      'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%A1%D0%BB%D0%B0%D0%B2%D1%8F%D0%BD%D1%81%D0%BA%202.JPG',
    'comfort_class':'https://raw.githubusercontent.com/alekyanrazmik-glitch/Projekt/main/%D0%A1%D0%B5%D0%BC%D0%B5%D0%B8%CC%86%D0%BD%D1%8B%D0%B8%CC%86%205.JPG',
    'kottedzh':    'https://raw.githubusercontent.com/alekyanrazmik-glitch/Projekt/main/%D0%98%D0%BB%D1%8C%D1%81%D0%BA%D0%B8%D0%B8%CC%86%203.JPG',
    'foundation':  'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%A1%D0%BB%D0%B0%D0%B2%D1%8F%D0%BD%D1%81%D0%BA%204.JPG',
    'roof':        'https://raw.githubusercontent.com/alekyanrazmik-glitch/Nashi-Raboti/main/%D0%9D%D0%BE%D0%B2%D0%BE%D1%80%D0%BE%D1%81%D1%81%D0%B8%D0%B8%CC%86%D1%81%D0%BA%203.JPG',
}
...
<section style="background:var(--bg-soft)">
  <div class="wrap">
    ...
    <div class="proj-grid" id="projGrid" data-limit="3"></div>
    <div style="text-align:center;margin-top:36px"><a href="/proekty.html" class="btn btn-dark btn-lg">Все проекты →</a></div>
  </div>
</section>
...
<section style="background:var(--bg-soft)">
  <div class="wrap">
    <h2 class="h-sec">Как мы строим в {pc}</h2>
    <p class="sub-sec">Прозрачный процесс с понятными контрольными точками — вы видите результат на каждом шаге.</p>
    <div class="stages">
      <div class="stage"><div class="sn">01</div><h3>Заявка и встреча</h3><p>Обсуждаем задачу, выезжаем на участок в {pc}, подбираем проект.</p></div>
      <div class="stage"><div class="sn">02</div><h3>Проект и смета</h3><p>Готовим проект, фиксируем цену и сроки в договоре.</p></div>
      <div class="stage"><div class="sn">03</div><h3>Фундамент</h3><p>Геология, разметка, устройство фундамента под грунт участка.</p></div>
      <div class="stage"><div class="sn">04</div><h3>Коробка и кровля</h3><p>Стены, перекрытия, монтаж кровельной системы.</p></div>
      <div class="stage"><div class="sn">05</div><h3>Инженерные сети</h3><p>Электрика, вода, канализация, отопление.</p></div>
      <div class="stage"><div class="sn">06</div><h3>Отделка</h3><p>Черновая или чистовая по комплектации.</p></div>
      <div class="stage"><div class="sn">07</div><h3>Сдача объекта</h3><p>Проверка по чек-листу и передача ключей.</p></div>
      <div class="stage"><div class="sn">08</div><h3>Гарантия</h3><p>10 лет на конструктив и сопровождение после сдачи.</p></div>
    </div>
  </div>
</section>
...
<section>
  <div class="wrap">
    <h2 class="h-sec">Наши построенные дома</h2>
    <p class="sub-sec">Объекты, сданные нашими собственными бригадами в Краснодарском крае. Аналогичный дом можем построить и в {pc} — нажмите на фото, чтобы открыть галерею.</p>
    <div class="obj-grid" id="objGrid" data-limit="3"></div>
    <div style="text-align:center;margin-top:36px"><a href="/obekty.html" class="btn btn-dark btn-lg">Все объекты →</a></div>
  </div>
</section>
...
<section style="background:var(--bg-soft)">
  <div class="wrap">
    <h2 class="h-sec">Отзывы клиентов</h2>
    <p class="sub-sec">Что говорят те, кто уже построил с нами дом в Краснодарском крае.</p>
    <div class="rev-grid">
      <div class="rev"><div class="stars">★★★★★</div><p>Переезжали из Сибири, дом строили дистанционно. Каждую неделю фото и отчёты. Смета не выросла ни на рубль, въехали в срок.</p><div class="rev-who"><div class="rev-ava">ИП</div><div><b>Игорь и Полина</b><span>дом 145 м², Геленджик</span></div></div></div>
      <div class="rev"><div class="stars">★★★★★</div><p>Подкупила прозрачность: всё по договору, оплата по этапам. Газобетонный дом получился тёплый и аккуратный.</p><div class="rev-who"><div class="rev-ava">СМ</div><div><b>Светлана М.</b><span>дом 110 м², Анапа</span></div></div></div>
      <div class="rev"><div class="stars">★★★★★</div><p>Платили по этапам — по факту работ. Бригады свои, без простоев, всё по договору. Спасибо за честную работу и нормальное общение.</p><div class="rev-who"><div class="rev-ava">АК</div><div><b>Алексей К.</b><span>дом 168 м², Краснодар</span></div></div></div>
    </div>
  </div>
</section>
...
<section>
  <div class="wrap">
    ...
    <div class="faq" id="faqList">
      <div class="fi"><div class="fq">Сколько стоит {title.lower()}?<span class="pl">+</span></div><div class="fa"><p>Точная стоимость зависит от площади, материала и комплектации. Базовые ориентиры мы привели выше; финальная цена фиксируется в договоре после выезда инженера на участок в {pc}.</p></div></div>
      <div class="fi"><div class="fq">Может ли измениться цена в процессе строительства?<span class="pl">+</span></div><div class="fa"><p>Нет. Мы работаем по фиксированной смете, закреплённой в договоре. Стоимость не меняется, если вы сами не вносите изменения в проект.</p></div></div>
      <div class="fi"><div class="fq">Сколько времени занимает строительство?<span class="pl">+</span></div><div class="fa"><p>Одноэтажный дом эконом-класса — от 3–4 месяцев, дом комфорт-класса под ключ — от 5–7 месяцев. Сроки фиксируются в договоре по этапам.</p></div></div>
      <div class="fi"><div class="fq">Вы выезжаете в {city}?<span class="pl">+</span></div><div class="fa"><p>Да, работаем по всему Краснодарскому краю. Выезд инженера на ваш участок в {pc} — бесплатно. Уточните адрес — рассчитаем логистику и сроки.</p></div></div>
      <div class="fi"><div class="fq">Как происходит оплата?<span class="pl">+</span></div><div class="fa"><p>Оплата поэтапная, по факту выполненных работ. Вы платите за каждый этап после его завершения, без предоплаты за весь дом сразу.</p></div></div>
      <div class="fi"><div class="fq">Какие документы вы предоставляете?<span class="pl">+</span></div><div class="fa"><p>Договор подряда с фиксированной сметой и сроками, акты выполненных работ по этапам, сертификаты на материалы и гарантийный паспорт на 10 лет.</p></div></div>
    </div>
  </div>
</section>
